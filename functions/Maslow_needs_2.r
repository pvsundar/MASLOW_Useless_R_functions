...
# V.2.2
# mODIFIED BY: SUNDAR BALAKRISHNAN
# DATE: JUNE 10, 2024
# 
...
useless_needs_pyramid <- function(
    ...
    # Center labels horizontally (x = 0) for better balance
    lab_df$label_x <- 0
    
    # Adaptive wrap width based on tier width
    # Top tiers are narrower, so use shorter wrap width
    lab_df$wrap_width <- round(lab_df$w_mid * 11)  # Scale wrap width to tier width
    
    # wrap long labels with adaptive width
    wrap_fun <- function(s, w) paste(strwrap(s, width = w), collapse = "\n")
    lab_df$label_wrapped <- mapply(wrap_fun, lab_df$label, lab_df$wrap_width, 
    ...
    poly_df <- merge(poly_df, transform(lab_df, tier = tier, fill = fill), 
                     by = "tier", sort = FALSE)
...
useless_needs_pyramid(
    levels = c(
        ...
        "Writing useless-useful R functions"
    ),
